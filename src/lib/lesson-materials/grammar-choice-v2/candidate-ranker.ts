...
import { grammarChoiceKnowledge, textbookFrequency } from "@/lib/lesson-materials/grammar-choice-v2/textbook-rules";
import type {
  ExactSentence,
  GrammarPriority,
  ResolvedCandidate,
...
export function rankCandidates(
  ...
  const sorted = [...items].sort((a, b) => {
    ...
    // (교재 반영 전 방식(ontology)은 예전처럼 위치 순서만 본다.)
    const tier = (code: string) =>
      knowledge === "ontology" ? 0 : Math.min(3, Math.floor(textbookFrequency(code) / 15));
