 ...
 * 토큰이라 비용이 작다.
 *
 * 복제 요청이 이기면 먼저 보낸 요청은 끊지만, 스트리밍이 아닌 호출은 끊어도 OpenAI가 끝까지
 * 만든 값을 받는다. 그래서 느린 호출마다 값이 두 배가 된다. 분석(GENERATOR)은 판정과 달리
 * 단계 상한이 없고 경로 상한(300초)까지 여유가 있어, 정말 멈춘 호출에만 복제가 붙게 45초로
 * 늦춘다(45초 + 복제 약 20초면 판정 단계까지 300초 안에 넉넉히 끝난다).
 */
const HEDGE_AFTER_MS: Record<"GENERATOR" | "REVIEWER", number> = {
  GENERATOR: 45_000,
  // 12초였을 때 판정 호출의 꼬리 부분마다 복제가 붙어 비용이 늘었다. 성공한 판정 대부분이 15초 안이다.
  // 판정 상한(35초) 안에 복제가 끝날 자리를 남겨야 해서 더 늦추지 않는다.
  REVIEWER: 20_000,
};
