...
/**
 * lesson_pack_json의 일부 필드만 바꿔 저장한다. 어법 선택·어휘 선택·워크북 제작이 같은 지문에
 * 동시에 캐시를 저장하므로, 읽은 뒤 누가 먼저 저장했으면(updated_at이 바뀌었으면) 다시 읽어
 * 합친다. 예전에는 나중 저장이 앞 저장을 덮어 캐시가 사라졌고, 다음 제작 때 같은 지문을 다시
 * 만들며 크레딧을 또 받았다(2026-09-15 어법·어휘 4지문 두 번 차감).
 */
