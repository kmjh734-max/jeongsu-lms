...
단어 수나 문장 수 할당을 쓰지 않는다. 원문에 해당 문법이 있고 정답이 하나면 문항을 만든다. 문장당 개수 상한은 없다. 지문당 최대 24개만 지킨다.
원문 문자를 수정·요약·삭제하지 말고, 원문에 있는 정확한 span만 정답으로 써라.
...
desiredCandidateCount는 지문 상한 24이다. 문장 수×2로 자르지 마라.
쉽지만 정답이 하나인 수일치·병렬·분사도 만들어라. visualize/visualizes, contradict/contradicts, holding/held는 구조가 분명하면 허용한다.
금지: to know/knowing, might be/been, could bear/bears, will have/has, have to come/comes.
범주를 문장마다 순서대로 대조하라: 문장구조, 수일치, 시제, 태, 조동사, to부정사·동명사, 분사, 병렬, 명사절, 관계사, 대명사, 형용사·부사, 비교, 가정법·도치·생략.
...
쉽다는 이유만으로 거절하지 않는다. 명확한 수일치, 병렬, 분사, 관계사도 정답이 하나면 승인한다.
to know/knowing, might be/been 같은 기계적 형태 대비와 둘 다 가능한 선택지는 거절한다.
