...
import {
  TYPE20_BLANK_SPEAKER,
  TYPE20_END_SPEAKER,
  buildType20Instruction,
...
export function fixType20Question(
  q: GeneratedListeningQuestion,
  typeId: number
): GeneratedListeningQuestion {
  if (typeId !== 20) return q;

  const segments = trimContinuationDialogue(q.segments, TYPE20_END_SPEAKER);
  const blank_speaker =
  ...
  const situation_type = normalizeSituationType(q.situation_type);

  return {
    ...q,
    order_index: 20,
    question_type: TYPE20_QUESTION_TYPE,
    instruction,
    question_text,
    segments,
    previous_turn,
