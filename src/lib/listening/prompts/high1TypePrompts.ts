...
형식: 포스터/장면 대화. ①~⑤ 요소를 사실로 언급. 그림은 그중 하나만 다르게 그림.
choices: ["①","②","③","④","⑤"] (채점용; 시험지에는 그림만, 보기는 그림 안 라벨).
needs_image_choices=true.
choice_image_prompts: 1장. 라벨 ①–⑤가 모두 보이고, 정답 라벨만 대화와 불일치.

논리(필수):
- 대화는 「포스터에 맞게 적혀 있어야 할 / 실제로 있다고 말하는」 올바른 사실을 말한다.
- 그림만 그중 하나를 틀리게 그린다.
- 금지: "포스터에는 6시로 되어 있는데 5시여야 해"처럼 대화가 이미 틀린 상태를 인정하는 말.
  (그러면 그림에 6시를 그린 것이 대화와 일치해 버려 앞뒤가 안 맞음)
- 예) 대화: "시작 시간은 5시야." / 그림 ⑤: 6:00 p.m. → 정답 ⑤
`.trim(),
  5: `
### 고1 5번 — 할 일
