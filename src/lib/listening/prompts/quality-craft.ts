...
/**
 * 실제로 나온 흠만 모은 목록. 검사에서 걸러 다시 만들기 전에, 쓸 때부터 피하게 한다
 * (2026-09-17~18 실측 — script-audit.ts가 같은 항목을 검사한다).
 */
const REAL_DEFECTS = `
[실제로 나왔던 흠 — 쓰기 전에 확인한다]
1. 같은 사람을 두 이름으로 부르지 않는다. 대화에 이름을 쓸 거면 한 이름만 쓴다.
2. 물건과 부속이 어울려야 한다. 주문한 물건에 없는 부품이 잘못 왔다고 쓰지 않는다.
3. 가진 것과 해결책이 맞아야 한다. 현금이 없다는 사람에게 지폐를 바꿔 주겠다고 하지 않는다.
4. 숫자·시각·거리를 스스로 계산해 본다. 걸리는 시간과 약속 시각이 맞아야 하고, 앞에서 거절한 값이
   결론이 되면 안 된다.
5. 담화(1인 말하기)는 첫 문장이 인사·안내로 시작한다. 앞 대화에 답하듯(That's right, …) 시작하거나
   한 사람만 부르며(Jinu, …) 시작하지 않는다.
6. 요구와 정답이 맞아야 한다. 더 짧은 문구를 묻는 대화라면 정답이 앞에 나온 문구보다 짧아야 한다.
7. 같은 담화로 두 문항을 만들 때(16·17번)는 두 문항의 대본이 글자 하나까지 같아야 한다.
`.trim();
