...
6. 영어가 틀린 문장: 그대로 두면 틀린 말이 되는 것만 고른다 — 문법이 어긋나거나, 그 뜻으로는 쓰지 않는
   짝(예: "on yesterday's school walk"), 한국어를 그대로 옮겨 뜻이 통하지 않는 말.
   "이렇게 쓰면 더 자연스럽다" 정도는 적지 않는다. 쉬운 말·짧은 말이라는 이유로도 적지 않는다
   (중학생 듣기 대본은 원래 쉽고 짧다).
7. 선택지가 시험 문항으로 성립하지 않음: 오답이 대본과 전혀 상관없어 듣지 않아도 지워지는 것이 둘 이상,
   한국어 선택지가 틀린 말이거나 뜻이 통하지 않음, 선택지끼리 뜻이 같아 둘 다 답이 될 수 있음.
8. 해설·정답 단서가 대본과 어긋남: 대본에 없는 말을 근거로 들거나, 해설이 가리키는 답이 정답 번호와 다름.
