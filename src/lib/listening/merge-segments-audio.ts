...
export async function mergeQuestionAudioFromSegments(opts: {
  ...
  try {
    for (const seg of segments) {
      if (!seg.audio_url) {
        throw new Error(
          `${seg.order_index + 1}번째 줄(${seg.speaker_type}) segment 음원이 없습니다. 먼저 「음원 생성」을 실행하세요.`
        );
      }
      const localPath = join(
        workDir,
        `${String(seg.order_index + 1).padStart(2, "0")}-${seg.speaker_type.toLowerCase()}.mp3`
      );
