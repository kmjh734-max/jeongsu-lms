...
export async function listStage5ItemsAction(passageId: string) {
  ...
  return { ok: true as const, items: (data ?? []) as ExamStage5Item[] };
}

export async function saveStage5ItemsAction(
  passageId: string,
  drafts: Stage5ItemDraft[]
) {
  const auth = await requireStaff();
  if (!auth.ok) return auth;
  const supabase = await createClient();
  ...
  for (const d of drafts) {
    ...
    const err = validateStage5ItemAgainstText(english, d);
    if (err) return { ok: false as const, message: err };
    warnings.push(...collectStage5Warnings(english, [d]));
  }

  const bySentence = new Map<string, Stage5ItemDraft[]>();
  for (const d of drafts) {
  ...
  for (const [, list] of bySentence) {
    const overlapWarn = collectStage5Warnings(
  ...
  for (let i = 0; i < drafts.length; i++) {
    ...
    const cues = parseCueWords(d.cue_words);
    const cats = parseGrammarCategories(d.grammar_category);
    const accepted = [...new Set((d.accepted_answers ?? []).map((a) => a.trim()).filter(Boolean))]
      .filter((a) => a.toLowerCase() !== d.answer_text.trim().toLowerCase());
...
export async function setStage5PublishedAction(
  passageId: string,
  published: boolean
) {
  const auth = await requireStaff();
  ...
  if (published) {
    const { data: items } = await supabase
      .from("exam_stage_blanks")
      .select("id, is_required, answer_text, cue_words, english_start, english_end, selected_text, sentence_id")
      .eq("passage_id", passageId)
      .eq("academy_id", auth.profile.academy_id)
      .eq("stage_number", 5);
    ...
    if (required.length < 1) {
      return {
        ok: false as const,
        message: "필수 동사형 항목이 최소 1개 있어야 공개할 수 있습니다.",
      };
    }
    ...
    for (const item of required) {
      ...
      const err = validateStage5ItemAgainstText(english, {
        ...
        cue_words: parseCueWords(item.cue_words),
      });
      if (err) {
        return {
          ok: false as const,
          message: `공개 전 범위 오류: ${err}`,
        };
      }
    }
  }
