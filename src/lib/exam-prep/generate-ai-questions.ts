...
import { buildStage6Drafts, buildStage6GrammarDrafts, buildStage6VocabDrafts, buildStage7Seed, mergeStage6Drafts } from "@/lib/exam-prep/auto-seed-stages";
...
export function stage6DraftsToQuestions(
...
export async function generateStepQuestionsWithAi(
  ...
  if (
    ...
    const plantDrafts =
      catFilter === "grammar"
        ? buildStage6GrammarDrafts(seedSentences)
        : catFilter === "vocabulary"
          ? buildStage6VocabDrafts(seedSentences)
          : buildStage6Drafts(seedSentences, "all");
    ...
    if (process.env.OPENAI_API_KEY?.trim()) {
      try {
        ...
        const useAi = ai.source === "ai" && ai.drafts.length > 0;
        const merged = mergeStage6Drafts(
          useAi ? ai.drafts : plantDrafts,
          plantDrafts,
          sentences.map((s) => s.id),
          {
            category: catFilter === "all" ? "all" : catFilter,
            minPerSentence: catFilter === "grammar" ? 2 : 1,
          }
        );
        return {
