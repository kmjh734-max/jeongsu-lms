...
/**
 * 이 문장에서 특히 살펴보라고 짚어준 코드. 정리된 온톨로지 전체는 시스템
 * 프롬프트에 실리므로, 라우팅이 맞게 걸리는지는 문장별 likelyCodes로 확인한다.
 */
function promptCodes(text: string): Set<string> {
  ...
  return new Set(payload.highlightedBySentence.flatMap((row) => row.likelyCodes));
}
